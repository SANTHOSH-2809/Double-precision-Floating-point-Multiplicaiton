...
module floatingpoint(
    ...
    // Detect subnormals
    wire a_subnormal = (exp_a == 0) && (frac_a != 0);
    wire b_subnormal = (exp_b == 0) && (frac_b != 0);

    // Treat subnormals as zero (based on your design rule)
    wire a_zero_eff = a_zero | a_subnormal;
    wire b_zero_eff = b_zero | b_subnormal;

    // Hidden bit insertion (only for normals)
    wire [52:0] mant_a = {1'b1, frac_a};
    wire [52:0] mant_b = {1'b1, frac_b};
    
    // Exponent calculation (unbiased)
    wire signed [12:0] exp_a_unbias = exp_a - 1023;
    wire signed [12:0] exp_b_unbias = exp_b - 1023;
    ...
    assign final_product =
        ...
        ((a_inf && b_zero_eff) || (a_zero_eff && b_inf)) ? 64'h7FF8000000000000 :
        (a_inf || b_inf) ? {sign_r,11'h7FF,52'd0} :
        (a_zero_eff || b_zero_eff) ? {sign_r,63'd0} :
        overflow ? {sign_r,11'h7FF,52'd0} :
        underflow ? {sign_r,63'd0} :
